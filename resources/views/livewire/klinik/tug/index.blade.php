<div>
    ...
    @section('breadcrumb')
        ...
        <li class="breadcrumb-item active">TUG</li>
    @endsection

    <h1 class="page-header">TUG <small>Tes Get Up and Go</small></h1>
                        ...
                        <tr>
                            ...
                            <td class="with-btn-group text-end" nowrap>
                                @role('administrator|supervisor|operator')
                                    ...
                                    @else
                                        @if ($row->tug->upload != '' || $row->tug->upload != null)
                                            <x-action :row="$row" custom="" :detail="false" :edit="false"
                                                :print="false" :permanentDelete="false" :restore="false" :delete="false" />
                                        @else
                                            <x-action :row="$row"
                                                custom="<li><hr class='dropdown-divider'></li><a href='javascript:;'class='dropdown-item fs-8px'>{{ $row->tug->user?->name }}<br>{{ $row->tug->updated_at }}</a>"
